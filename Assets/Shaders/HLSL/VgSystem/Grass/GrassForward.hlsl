#ifndef M_GRASS_FORWARD_PASS_INCLUDED
#define M_GRASS_FORWARD_PASS_INCLUDED

#include "Assets/M_Pipeline/ShaderLibrary/M_Lighting.hlsl"
...
// #endif
    ...
    float2 aoUV : TEXCOORD9;
    float4 positionCS               : SV_POSITION;
    float4 positionOS               : TEXCOORD10;
    float4 windLineColor            : TEXCOORD11;
...
Varyings LitPassVertex(Attributes input)
{
    ...
    float3 windOffset = WindOffset(input.positionOS);
    input.positionOS.xyz += windOffset;    

//#endif
    
    output.windLineColor.rgb = windOffset;
...
)
{
    ...
    BlendTerrainOutput blendTerrainOutput;
        ...
        float2 uv1 = float2(input.positionWS.x,input.positionWS.z) / ( half2(2,2) / mossUV_xy);
        float2 uv2 = mossUV_zw / mossUV_xy;
        float2 finalMossUV = uv1 + uv2;
        half4 mossColor = SampleAlbedoAlpha(finalMossUV,TEXTURE2D_ARGS(_MossBase,sampler_MossBase));
        half4 groundColor = mossColor * 1;
        M_BlendMoss(input.tangentWS,input.normalWS,input.positionWS, _BlendRange,surfaceData,mossColor,blendTerrainOutput);
    #else
    ...
    half interColor = input.windLineColor.a;
    half l = saturate(input.positionOS.y) * interColor;
