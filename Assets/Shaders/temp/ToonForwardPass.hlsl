...
half3 GetAdditionalLightDiffuse(Varyings input, Light light, half3 albedo, half aoFactor)
{
    half NDotL = dot(input.normalWS, light.direction);
    half wrapped = saturate((NDotL + _AdditionalLightWrap) / (1.0 + _AdditionalLightWrap));
    half attenuation = light.distanceAttenuation * lerp(1.0h, light.shadowAttenuation, _AdditionalLightShadowStrength);
    half lit = wrapped * attenuation * aoFactor * _AdditionalLightDiffuseIntensity;
    return albedo * light.color * lit;
}

half3 GetAdditionalLightSpecular(Varyings input, Light light, half3 albedo, half3 lightMap)
{
    half3 V = GetWorldSpaceNormalizeViewDir(input.positionWS);
    half3 H = SafeNormalize(light.direction + V);
    half NDotH = saturate(dot(input.normalWS, H));
    half blinnPhong = pow(NDotH, max(_AdditionalSpecularSmoothness, 1.0h));
    half attenuation = light.distanceAttenuation * lerp(1.0h, light.shadowAttenuation, _AdditionalLightShadowStrength);

    half nonMetalMask = step(lightMap.r, 0.9h);
    half3 nonMetalSpec = blinnPhong * light.color * attenuation * lightMap.r * _AdditionalLightSpecularIntensity * nonMetalMask;

    half metalMask = step(0.9h, lightMap.r);
    half3 metalSpec = blinnPhong * light.color * attenuation * albedo * lightMap.b * _AdditionalLightSpecularIntensity * metalMask;
...
half4 ForwardPassFragment(Varyings input, FRONT_FACE_TYPE facing : FRONT_FACE_SEMANTIC) : SV_TARGET
{
    ...
    half3 additionalDiffuse = 0.0;
    half3 additionalSpecular = 0.0;
    ...
    LIGHT_LOOP_BEGIN(pixelLightCount)
        ...
        half aoFactor = lightMap.g * input.color.r;
#if _IS_FACE
        half faceLightWeight = _AdditionalLightAffectsFace;
#else
        half faceLightWeight = 1.0h;
#endif
        additionalDiffuse += GetAdditionalLightDiffuse(input, additionalLight, albedo, aoFactor) * faceLightWeight;
#if _SPECULAR
        additionalSpecular += GetAdditionalLightSpecular(input, additionalLight, albedo, lightMap.rgb) * faceLightWeight;
#endif
    LIGHT_LOOP_END
#endif

    half3 finalColor = albedo * shadowColor + additionalDiffuse + specular + additionalSpecular + rim + emission;
    half finalAlpha = 1.0;
